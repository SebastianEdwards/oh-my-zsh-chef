...
node['oh_my_zsh']['users'].each do |user_hash|
  ...
  home_directory = "/home/#{user_hash[:login]}" unless home_directory.length > 0
  ...
  execute "source /etc/profile to all zshrc" do
    command "echo 'source /etc/profile' >> /etc/zprofile"
    not_if "grep 'source /etc/profile' /etc/zprofile"
  end
